...
//       
//       Creation Date:  Mon Oct 30 14:39:22 2023
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_dp_512x28 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_dp_512x28 (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA,
...
`endif
  ...
  parameter WORDS = 512;
  parameter MUX = 16;
  parameter MEM_WIDTH = 448; // redun block size 4, 224 on left, 224 on right
  parameter MEM_HEIGHT = 32;
  parameter WP_SIZE = 28 ;
...
`endif
  ...
  reg [447:0] new_data;
  reg [447:0] data_out;
  reg [111:0] readLatch0;
  reg [111:0] shifted_readLatch0;
  reg [1:0] read_mux_sel0;
  reg [111:0] readLatch1;
  reg [111:0] shifted_readLatch1;
  reg [1:0] read_mux_sel1;
  reg LAST_CLKB;
  reg [27:0] QA_int;
  reg [27:0] QA_int_delayed;
  reg [27:0] QB_int;
  reg [27:0] QB_int_delayed;
  reg [27:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
  ...
  reg NOT_AA1, NOT_AA0, NOT_DA27, NOT_DA26, NOT_DA25, NOT_DA24, NOT_DA23, NOT_DA22;
  ...
  reg NOT_DA4, NOT_DA3, NOT_DA2, NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB, NOT_AB8, NOT_AB7;
  reg NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB27, NOT_DB26;
  reg NOT_DB25, NOT_DB24, NOT_DB23, NOT_DB22, NOT_DB21, NOT_DB20, NOT_DB19, NOT_DB18;
  reg NOT_DB17, NOT_DB16, NOT_DB15, NOT_DB14, NOT_DB13, NOT_DB12, NOT_DB11, NOT_DB10;
  reg NOT_DB9, NOT_DB8, NOT_DB7, NOT_DB6, NOT_DB5, NOT_DB4, NOT_DB3, NOT_DB2, NOT_DB1;
  reg NOT_DB0, NOT_EMAA2, NOT_EMAA1, NOT_EMAA0, NOT_EMAWA1, NOT_EMAWA0, NOT_EMASA;
  ...
  reg NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA27, NOT_TDA26, NOT_TDA25, NOT_TDA24, NOT_TDA23;
  ...
  reg NOT_TENB, NOT_TCENB, NOT_TWENB, NOT_TAB8, NOT_TAB7, NOT_TAB6, NOT_TAB5, NOT_TAB4;
  reg NOT_TAB3, NOT_TAB2, NOT_TAB1, NOT_TAB0, NOT_TDB27, NOT_TDB26, NOT_TDB25, NOT_TDB24;
  reg NOT_TDB23, NOT_TDB22, NOT_TDB21, NOT_TDB20, NOT_TDB19, NOT_TDB18, NOT_TDB17;
  reg NOT_TDB16, NOT_TDB15, NOT_TDB14, NOT_TDB13, NOT_TDB12, NOT_TDB11, NOT_TDB10;
  reg NOT_TDB9, NOT_TDB8, NOT_TDB7, NOT_TDB6, NOT_TDB5, NOT_TDB4, NOT_TDB3, NOT_TDB2;
  reg NOT_TDB1, NOT_TDB0, NOT_RET1N, NOT_STOVA, NOT_STOVB, NOT_COLLDISN;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {28{1'bx}};
  ...
  assign `ARM_UD_DP DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {28{1'bx}};
  assign `ARM_UD_SEQ QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {28{1'bx}};
  assign `ARM_UD_SEQ QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {28{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {28{1'b1}};
        row_mask =  ( {15'b000000000000000, writeEnable[27], 15'b000000000000000, writeEnable[26],
        ...
        new_data =  ( {15'b000000000000000, wordtemp[27], 15'b000000000000000, wordtemp[26],
	...
	dump_file_desc = $fopen(filename_dump);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        ...
        readLatch0 = {data_out[444], data_out[440], data_out[436], data_out[432], data_out[428],
      ...
      QA_int = {shifted_readLatch0[108], shifted_readLatch0[104], shifted_readLatch0[100],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENA_int)} === 1'bx) begin
      ...
      QA_int = WENA_int !== 1'b1 ? QA_int : {28{1'bx}};
      QA_int_delayed = WENA_int !== 1'b1 ? QA_int_delayed : {28{1'bx}};
      ...
      if (WENA_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[27], 15'b000000000000000, writeEnable[26],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DA_int[27], 15'b000000000000000, DA_int[26],
          15'b000000000000000, DA_int[25], 15'b000000000000000, DA_int[24], 15'b000000000000000, DA_int[23],
          15'b000000000000000, DA_int[22], 15'b000000000000000, DA_int[21], 15'b000000000000000, DA_int[20],
          15'b000000000000000, DA_int[19], 15'b000000000000000, DA_int[18], 15'b000000000000000, DA_int[17],
          15'b000000000000000, DA_int[16], 15'b000000000000000, DA_int[15], 15'b000000000000000, DA_int[14],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENB_int)} === 1'bx) begin
      ...
      QB_int = WENB_int !== 1'b1 ? QB_int : {28{1'bx}};
      QB_int_delayed = WENB_int !== 1'b1 ? QB_int_delayed : {28{1'bx}};
      ...
      if (WENB_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[27], 15'b000000000000000, writeEnable[26],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DB_int[27], 15'b000000000000000, DB_int[26],
          15'b000000000000000, DB_int[25], 15'b000000000000000, DB_int[24], 15'b000000000000000, DB_int[23],
          15'b000000000000000, DB_int[22], 15'b000000000000000, DB_int[21], 15'b000000000000000, DB_int[20],
          15'b000000000000000, DB_int[19], 15'b000000000000000, DB_int[18], 15'b000000000000000, DB_int[17],
          15'b000000000000000, DB_int[16], 15'b000000000000000, DB_int[15], 15'b000000000000000, DB_int[14],
        ...
        data_out = (row >> (mux_address%4));
        readLatch1 = {data_out[444], data_out[440], data_out[436], data_out[432], data_out[428],
          data_out[424], data_out[420], data_out[416], data_out[412], data_out[408],
          data_out[404], data_out[400], data_out[396], data_out[392], data_out[388],
          data_out[384], data_out[380], data_out[376], data_out[372], data_out[368],
          data_out[364], data_out[360], data_out[356], data_out[352], data_out[348],
      ...
      QB_int = {shifted_readLatch1[108], shifted_readLatch1[104], shifted_readLatch1[100],
        shifted_readLatch1[96], shifted_readLatch1[92], shifted_readLatch1[88], shifted_readLatch1[84],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B39(CENYB, CENYB_);
  buf B40(WENYB, WENYB_);
  buf B41(AYB[0], AYB_[0]);
  buf B42(AYB[1], AYB_[1]);
  buf B43(AYB[2], AYB_[2]);
  buf B44(AYB[3], AYB_[3]);
  buf B45(AYB[4], AYB_[4]);
  buf B46(AYB[5], AYB_[5]);
  buf B47(AYB[6], AYB_[6]);
  buf B48(AYB[7], AYB_[7]);
  buf B49(AYB[8], AYB_[8]);
  buf B50(DYB[0], DYB_[0]);
  buf B51(DYB[1], DYB_[1]);
  buf B52(DYB[2], DYB_[2]);
  buf B53(DYB[3], DYB_[3]);
  buf B54(DYB[4], DYB_[4]);
  buf B55(DYB[5], DYB_[5]);
  buf B56(DYB[6], DYB_[6]);
  buf B57(DYB[7], DYB_[7]);
  buf B58(DYB[8], DYB_[8]);
  buf B59(DYB[9], DYB_[9]);
  buf B60(DYB[10], DYB_[10]);
  buf B61(DYB[11], DYB_[11]);
  buf B62(DYB[12], DYB_[12]);
  buf B63(DYB[13], DYB_[13]);
  buf B64(DYB[14], DYB_[14]);
  buf B65(DYB[15], DYB_[15]);
  buf B66(DYB[16], DYB_[16]);
  buf B67(DYB[17], DYB_[17]);
  buf B68(DYB[18], DYB_[18]);
  buf B69(DYB[19], DYB_[19]);
  buf B70(DYB[20], DYB_[20]);
  buf B71(DYB[21], DYB_[21]);
  buf B72(DYB[22], DYB_[22]);
  buf B73(DYB[23], DYB_[23]);
  buf B74(DYB[24], DYB_[24]);
  buf B75(DYB[25], DYB_[25]);
  buf B76(DYB[26], DYB_[26]);
  buf B77(DYB[27], DYB_[27]);
  buf B78(QA[0], QA_[0]);
  buf B79(QA[1], QA_[1]);
  buf B80(QA[2], QA_[2]);
  buf B81(QA[3], QA_[3]);
  ...
  buf B114(QB[8], QB_[8]);
  buf B115(QB[9], QB_[9]);
  buf B116(QB[10], QB_[10]);
  buf B117(QB[11], QB_[11]);
  buf B118(QB[12], QB_[12]);
  buf B119(QB[13], QB_[13]);
  buf B120(QB[14], QB_[14]);
  buf B121(QB[15], QB_[15]);
  buf B122(QB[16], QB_[16]);
  buf B123(QB[17], QB_[17]);
  buf B124(QB[18], QB_[18]);
  buf B125(QB[19], QB_[19]);
  buf B126(QB[20], QB_[20]);
  buf B127(QB[21], QB_[21]);
  buf B128(QB[22], QB_[22]);
  buf B129(QB[23], QB_[23]);
  buf B130(QB[24], QB_[24]);
  buf B131(QB[25], QB_[25]);
  buf B132(QB[26], QB_[26]);
  buf B133(QB[27], QB_[27]);
  buf B134(CLKA_, CLKA);
  buf B135(CENA_, CENA);
  buf B136(WENA_, WENA);
  buf B137(AA_[0], AA[0]);
  ...
  buf B154(DA_[8], DA[8]);
  buf B155(DA_[9], DA[9]);
  buf B156(DA_[10], DA[10]);
  buf B157(DA_[11], DA[11]);
  buf B158(DA_[12], DA[12]);
  buf B159(DA_[13], DA[13]);
  buf B160(DA_[14], DA[14]);
  buf B161(DA_[15], DA[15]);
  buf B162(DA_[16], DA[16]);
  buf B163(DA_[17], DA[17]);
  buf B164(DA_[18], DA[18]);
  buf B165(DA_[19], DA[19]);
  buf B166(DA_[20], DA[20]);
  buf B167(DA_[21], DA[21]);
  buf B168(DA_[22], DA[22]);
  buf B169(DA_[23], DA[23]);
  buf B170(DA_[24], DA[24]);
  buf B171(DA_[25], DA[25]);
  buf B172(DA_[26], DA[26]);
  buf B173(DA_[27], DA[27]);
  buf B174(CLKB_, CLKB);
  buf B175(CENB_, CENB);
  buf B176(WENB_, WENB);
  buf B177(AB_[0], AB[0]);
  ...
  buf B194(DB_[8], DB[8]);
  buf B195(DB_[9], DB[9]);
  buf B196(DB_[10], DB[10]);
  buf B197(DB_[11], DB[11]);
  buf B198(DB_[12], DB[12]);
  buf B199(DB_[13], DB[13]);
  buf B200(DB_[14], DB[14]);
  buf B201(DB_[15], DB[15]);
  buf B202(DB_[16], DB[16]);
  buf B203(DB_[17], DB[17]);
  buf B204(DB_[18], DB[18]);
  buf B205(DB_[19], DB[19]);
  buf B206(DB_[20], DB[20]);
  buf B207(DB_[21], DB[21]);
  buf B208(DB_[22], DB[22]);
  buf B209(DB_[23], DB[23]);
  buf B210(DB_[24], DB[24]);
  buf B211(DB_[25], DB[25]);
  buf B212(DB_[26], DB[26]);
  buf B213(DB_[27], DB[27]);
  buf B214(EMAA_[0], EMAA[0]);
  buf B215(EMAA_[1], EMAA[1]);
  buf B216(EMAA_[2], EMAA[2]);
  buf B217(EMAWA_[0], EMAWA[0]);
  buf B218(EMAWA_[1], EMAWA[1]);
  buf B219(EMASA_, EMASA);
  buf B220(EMAB_[0], EMAB[0]);
  buf B221(EMAB_[1], EMAB[1]);
  buf B222(EMAB_[2], EMAB[2]);
  buf B223(EMAWB_[0], EMAWB[0]);
  buf B224(EMAWB_[1], EMAWB[1]);
  buf B225(EMASB_, EMASB);
  buf B226(TENA_, TENA);
  buf B227(BENA_, BENA);
  buf B228(TCENA_, TCENA);
  buf B229(TWENA_, TWENA);
  buf B230(TAA_[0], TAA[0]);
  buf B231(TAA_[1], TAA[1]);
  buf B232(TAA_[2], TAA[2]);
  buf B233(TAA_[3], TAA[3]);
  buf B234(TAA_[4], TAA[4]);
  buf B235(TAA_[5], TAA[5]);
  buf B236(TAA_[6], TAA[6]);
  buf B237(TAA_[7], TAA[7]);
  buf B238(TAA_[8], TAA[8]);
  buf B239(TDA_[0], TDA[0]);
  buf B240(TDA_[1], TDA[1]);
  buf B241(TDA_[2], TDA[2]);
  buf B242(TDA_[3], TDA[3]);
  buf B243(TDA_[4], TDA[4]);
  buf B244(TDA_[5], TDA[5]);
  buf B245(TDA_[6], TDA[6]);
  buf B246(TDA_[7], TDA[7]);
  buf B247(TDA_[8], TDA[8]);
  buf B248(TDA_[9], TDA[9]);
  buf B249(TDA_[10], TDA[10]);
  buf B250(TDA_[11], TDA[11]);
  buf B251(TDA_[12], TDA[12]);
  buf B252(TDA_[13], TDA[13]);
  buf B253(TDA_[14], TDA[14]);
  buf B254(TDA_[15], TDA[15]);
  buf B255(TDA_[16], TDA[16]);
  buf B256(TDA_[17], TDA[17]);
  buf B257(TDA_[18], TDA[18]);
  buf B258(TDA_[19], TDA[19]);
  buf B259(TDA_[20], TDA[20]);
  buf B260(TDA_[21], TDA[21]);
  buf B261(TDA_[22], TDA[22]);
  buf B262(TDA_[23], TDA[23]);
  buf B263(TDA_[24], TDA[24]);
  buf B264(TDA_[25], TDA[25]);
  buf B265(TDA_[26], TDA[26]);
  buf B266(TDA_[27], TDA[27]);
  buf B267(TQA_[0], TQA[0]);
  buf B268(TQA_[1], TQA[1]);
  buf B269(TQA_[2], TQA[2]);
  buf B270(TQA_[3], TQA[3]);
  buf B271(TQA_[4], TQA[4]);
  buf B272(TQA_[5], TQA[5]);
  buf B273(TQA_[6], TQA[6]);
  buf B274(TQA_[7], TQA[7]);
  buf B275(TQA_[8], TQA[8]);
  buf B276(TQA_[9], TQA[9]);
  buf B277(TQA_[10], TQA[10]);
  buf B278(TQA_[11], TQA[11]);
  buf B279(TQA_[12], TQA[12]);
  buf B280(TQA_[13], TQA[13]);
  buf B281(TQA_[14], TQA[14]);
  buf B282(TQA_[15], TQA[15]);
  buf B283(TQA_[16], TQA[16]);
  buf B284(TQA_[17], TQA[17]);
  buf B285(TQA_[18], TQA[18]);
  buf B286(TQA_[19], TQA[19]);
  buf B287(TQA_[20], TQA[20]);
  buf B288(TQA_[21], TQA[21]);
  buf B289(TQA_[22], TQA[22]);
  buf B290(TQA_[23], TQA[23]);
  buf B291(TQA_[24], TQA[24]);
  buf B292(TQA_[25], TQA[25]);
  buf B293(TQA_[26], TQA[26]);
  buf B294(TQA_[27], TQA[27]);
  buf B295(TENB_, TENB);
  buf B296(BENB_, BENB);
  buf B297(TCENB_, TCENB);
  buf B298(TWENB_, TWENB);
  buf B299(TAB_[0], TAB[0]);
  buf B300(TAB_[1], TAB[1]);
  buf B301(TAB_[2], TAB[2]);
  buf B302(TAB_[3], TAB[3]);
  buf B303(TAB_[4], TAB[4]);
  buf B304(TAB_[5], TAB[5]);
  buf B305(TAB_[6], TAB[6]);
  buf B306(TAB_[7], TAB[7]);
  buf B307(TAB_[8], TAB[8]);
  buf B308(TDB_[0], TDB[0]);
  buf B309(TDB_[1], TDB[1]);
  buf B310(TDB_[2], TDB[2]);
  buf B311(TDB_[3], TDB[3]);
  buf B312(TDB_[4], TDB[4]);
  buf B313(TDB_[5], TDB[5]);
  buf B314(TDB_[6], TDB[6]);
  buf B315(TDB_[7], TDB[7]);
  buf B316(TDB_[8], TDB[8]);
  buf B317(TDB_[9], TDB[9]);
  buf B318(TDB_[10], TDB[10]);
  buf B319(TDB_[11], TDB[11]);
  buf B320(TDB_[12], TDB[12]);
  buf B321(TDB_[13], TDB[13]);
  buf B322(TDB_[14], TDB[14]);
  buf B323(TDB_[15], TDB[15]);
  buf B324(TDB_[16], TDB[16]);
  buf B325(TDB_[17], TDB[17]);
  buf B326(TDB_[18], TDB[18]);
  buf B327(TDB_[19], TDB[19]);
  buf B328(TDB_[20], TDB[20]);
  buf B329(TDB_[21], TDB[21]);
  buf B330(TDB_[22], TDB[22]);
  buf B331(TDB_[23], TDB[23]);
  buf B332(TDB_[24], TDB[24]);
  buf B333(TDB_[25], TDB[25]);
  buf B334(TDB_[26], TDB[26]);
  buf B335(TDB_[27], TDB[27]);
  buf B336(TQB_[0], TQB[0]);
  buf B337(TQB_[1], TQB[1]);
  buf B338(TQB_[2], TQB[2]);
  buf B339(TQB_[3], TQB[3]);
  buf B340(TQB_[4], TQB[4]);
  buf B341(TQB_[5], TQB[5]);
  buf B342(TQB_[6], TQB[6]);
  buf B343(TQB_[7], TQB[7]);
  buf B344(TQB_[8], TQB[8]);
  buf B345(TQB_[9], TQB[9]);
  buf B346(TQB_[10], TQB[10]);
  buf B347(TQB_[11], TQB[11]);
  buf B348(TQB_[12], TQB[12]);
  buf B349(TQB_[13], TQB[13]);
  buf B350(TQB_[14], TQB[14]);
  buf B351(TQB_[15], TQB[15]);
  buf B352(TQB_[16], TQB[16]);
  buf B353(TQB_[17], TQB[17]);
  buf B354(TQB_[18], TQB[18]);
  buf B355(TQB_[19], TQB[19]);
  buf B356(TQB_[20], TQB[20]);
  buf B357(TQB_[21], TQB[21]);
  buf B358(TQB_[22], TQB[22]);
  buf B359(TQB_[23], TQB[23]);
  buf B360(TQB_[24], TQB[24]);
  buf B361(TQB_[25], TQB[25]);
  buf B362(TQB_[26], TQB[26]);
  buf B363(TQB_[27], TQB[27]);
  buf B364(RET1N_, RET1N);
  buf B365(STOVA_, STOVA);
  buf B366(STOVB_, STOVB);
  buf B367(COLLDISN_, COLLDISN);
  ...
  assign DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {28{1'bx}};
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {28{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_dp_512x28_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {28{1'bx}};
  `endif
  assign QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {28{1'bx}};
...
`endif
`timescale 1ns/1ps
module sram_dp_512x28_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [27:0] Q_out;
   input [27:0] Q_in;
   input CLK;
   input [8:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(9'd505,5'd18,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 13, column address
   //should have value in range of 12 to 15
   //In order to inject fault in redundant column for Bit 14 to 27, column address
